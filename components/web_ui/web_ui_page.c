...
".dx-mode-toggle{display:flex;gap:10px;margin-bottom:16px;flex-wrap:wrap;align-items:center;}"
".dx-mode-toggle button{background:#0c0e12;border:1px solid #1f2430;color:#cbd5e1;border-radius:10px;padding:8px 16px;cursor:pointer;font-weight:600;transition:all .2s;}"
".dx-mode-toggle button.active{border-color:#3b82f6;background:#13203d;color:#fff;box-shadow:0 8px 18px rgba(59,130,246,0.35);}"
...
"</style><link rel='stylesheet' href='/ui/devices.css?v=3'><link rel='stylesheet' href='/ui/devices_editor.css?v=3'></head><body>"
        "<div class='page'>"
...
"<div class='dx-mode-toggle' id='device_editor_toggle'><button type='button' data-mode='simple' class='active'>Simple editor</button><button type='button' data-mode='wizard'>Advanced wizard</button></div>"
"<div id='device_editor_root' class='dx-editor-pane active'></div>"
"<div id='device_wizard_root' class='dx-editor-pane'></div>"
"</div>"
"<script>"
...
"const loadedTabs={status:true,audio:false,devices:false};"
...
"function ensureTabLoaded(tab){switch(tab){case 'audio':if(!loadedTabs.audio){loadFiles().then(()=>{loadedTabs.audio=true;}).catch(()=>{});}break;case 'devices':loadedTabs.devices=true;break;default:break;}}"
"let currentDeviceEditorMode='simple';"
"function setDeviceEditorMode(mode){const normalized=mode==='wizard'?'wizard':'simple';currentDeviceEditorMode=normalized;const toggle=document.getElementById('device_editor_toggle');if(toggle){toggle.querySelectorAll('button[data-mode]').forEach(btn=>{btn.classList.toggle('active',btn.dataset.mode===normalized);});}const simplePane=document.getElementById('device_editor_root');if(simplePane){simplePane.classList.toggle('active',normalized==='simple');}const wizardPane=document.getElementById('device_wizard_root');if(wizardPane){wizardPane.classList.toggle('active',normalized==='wizard');}}"
"function initDeviceEditorToggle(){const toggle=document.getElementById('device_editor_toggle');if(!toggle){return;}toggle.addEventListener('click',ev=>{const btn=ev.target.closest('button[data-mode]');if(!btn){return;}setDeviceEditorMode(btn.dataset.mode);});setDeviceEditorMode(currentDeviceEditorMode);}"
"initDeviceEditorToggle();"
...
"function renderStatus(j){setTxt('status_raw',JSON.stringify(j,null,2));setTxt('status_ssid',j.wifi.ssid||'n/a');setTxt('status_ip',j.wifi.sta_ip||'none');const sd=j.sd||{};if(sd.ok){setTxt('status_sd_size',`${humanGb(sd.total)} / free ${humanGb(sd.free)}`);}else{setTxt('status_sd_size','No card');}setTxt('status_clients',j.clients?j.clients.total:0);renderUidMonitor(j.uid_monitor||[]);}"
...
"</script><script src='/ui/devices_editor.js?v=3'></script><script src='/ui/devices.js?v=3'></script>"
        "</div>"
        "</body></html>";
