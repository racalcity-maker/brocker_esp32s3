...
"function renderStatus(j){setTxt('status_raw',JSON.stringify(j,null,2));setTxt('status_ssid',j.wifi.ssid||'n/a');setTxt('status_ip',j.wifi.sta_ip||'none');const sd=j.sd||{};if(sd.ok){setTxt('status_sd_size',`${humanGb(sd.total)} / free ${humanGb(sd.free)}`);}else{setTxt('status_sd_size','No card');}setTxt('status_clients',j.clients?j.clients.total:0);const webUserInput=document.getElementById('web_user');if(webUserInput){webUserInput.value=(j.web&&j.web.username)||'';}const diagBox=document.getElementById('diag_verbose');if(diagBox){diagBox.checked=!!(j.diag&&j.diag.verbose_logging);}renderUidMonitor(j.uid_monitor||[]);}"
...
"const PAD_COLORS=["
"  {color:'#c084fc',border:'#a855f7',glow:'rgba(169,85,247,0.55)'},"
"  {color:'#34d399',border:'#10b981',glow:'rgba(16,185,129,0.5)'},"
"  {color:'#38bdf8',border:'#0ea5e9',glow:'rgba(14,165,233,0.55)'},"
"  {color:'#fbbf24',border:'#f59e0b',glow:'rgba(245,158,11,0.5)'},"
"  {color:'#f472b6',border:'#ec4899',glow:'rgba(236,72,153,0.5)'}"
"];"
